...
class CSPFlow(BaseModule):
    ...
    def forward(self, batch, guide_threshold=None):
        ...
        # lattice
        if self.lattice_polar:
            lattices_rep_T = batch.lattice_polar[:bootstrap_batchsize]
            lattices_rep_0 = self.sample_lattice_polar(bootstrap_batchsize)
            lattices_mat_T = lattice_polar_build_torch(lattices_rep_T)
        else:
            raise RuntimeError("Not implemented")

        # coords
        mask = (batch.batch < bootstrap_batchsize) 
        bootstrap_num_nodes = mask.sum()
        frac_coords = batch.frac_coords[mask]
        f0 = torch.rand_like(frac_coords)

        # types
        if self.pred_type:
            if self.type_encoding is None:
                gt_atom_types_onehot = F.one_hot(batch.atom_types[mask] - 1, num_classes=MAX_ATOMIC_NUM).float()
                rd_atom_types_onehot = torch.randn_like(gt_atom_types_onehot)
            else:
                gt_atom_types_onehot = self.type_encoding(batch.atom_types[mask])
                rd_atom_types_onehot = self.type_encoding.get_rd_encoded_types(bootstrap_num_nodes, device=self.device)
        ...
        input_frac_coords = f0 + times.repeat_interleave(batch.num_atoms[:bootstrap_batchsize])[:, None] * tar_f
        if self.lattice_polar:
            input_lattice_mat = lattice_polar_build_torch(input_lattice_rep)
        else:
            input_lattice_mat = input_lattice_rep
        if self.pred_type:
            input_atom_type_probs = rd_atom_types_onehot + times.repeat_interleave(batch.num_atoms[:bootstrap_batchsize])[:, None] * tar_t
            input_atom_types = input_atom_type_probs
        else:
            input_atom_types = batch.atom_types[:bootstrap_num_nodes]
        ...
        vb1 = self.decoder(
            ...
            node2graph=batch.batch[:bootstrap_num_nodes],
            lattices_mat=input_lattice_mat,
            cemb=cemb, guide_indicator=guide_indicator,
        )
        if self.pred_type:
            vb1_l, vb1_f, vb1_t = vb1
        else:
            vb1_l, vb1_f = vb1

        t2 = times + dt_bootstrap # [bootstrap_batchsize]
        ...
        vb2 = self.decoder(
            ...
            num_atoms=batch.num_atoms[:bootstrap_batchsize],
            node2graph=batch.batch[:bootstrap_num_nodes],
            lattices_mat=lattices_mat_t2,
            cemb=cemb, guide_indicator=guide_indicator,
        )
        if self.pred_type:
            vb2_l, vb2_f, vb2_t = vb2
        else:
            vb2_l, vb2_f = vb2

        v_target_l = (vb1_l + vb2_l)/2
        ...
        if self.lattice_polar:
            lattices_rep_T_flow = batch.lattice_polar[bootstrap_batchsize:]
            lattices_rep_0_flow = self.sample_lattice_polar(flow_batchsize)
            lattices_mat_T_flow = lattice_polar_build_torch(lattices_rep_T_flow)
        else:
            raise RuntimeError("Not implemented")
        ...
        f0_flow = torch.rand_like(frac_coords_flow)
        ...
        input_lattice_rep_flow = lattices_rep_0_flow + t_flow[l_expand_dim_flow] * tar_l_flow
        input_frac_coords_flow = f0_flow + t_flow.repeat_interleave(batch.num_atoms[bootstrap_batchsize:])[:, None] * tar_f_flow
        if self.lattice_polar:
            input_lattice_mat_flow = lattice_polar_build_torch(input_lattice_rep_flow)
        else:
            raise RuntimeError("Not implemented")
        if self.pred_type:
        ...
        # Replace inputs if fixed
        if self.keep_coords:
            input_frac_coords_flow = frac_coords_flow
        if self.keep_lattice:
            input_lattice_rep_flow = lattices_rep_T_flow
        ...
        input_atom_types_concat = torch.cat([input_atom_types, input_atom_types_flow], dim=0)
        input_frac_coords_concat = torch.cat([input_frac_coords, input_frac_coords_flow], dim=0)
        input_lattice_rep_concat = torch.cat([input_lattice_rep, input_lattice_rep_flow], dim=0)
        ...
        pred_concat = self.decoder(
            t=t_emb_concat,
            dt=dt_emb_concat,
            atom_types=input_atom_types_concat,
            frac_coords=input_frac_coords_concat,
            lattices_rep=input_lattice_rep_concat,
            num_atoms=batch.num_atoms,
            node2graph=batch.batch,
            lattices_mat=input_lattice_mat_concat,
            cemb=cemb, guide_indicator=guide_indicator,
        )
        if self.pred_type:
            pred_l, pred_f, pred_t = pred_concat
        else:
            pred_l, pred_f = pred_concat
