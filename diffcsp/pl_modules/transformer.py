...
from diffcsp.pl_modules.cspnet import SinusoidsEmbedding, RecSinusoidsEmbedding
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        '''
        node_features: [nnode, hidden_dim]
        lattices_rep: [nbz, 6 or 9]
        edge_index: [2, nedge]
        frac_diff: [nedge, 3]
        '''
        h = node_features
        ...
        kv_input.append(lattice_ips_flatten_edges)
        ...
        attn_logits = torch.einsum('hed, hed->he', q, k) / (self.head_dim ** 0.5) # [num_heads, nedge]
        attn_weights = scatter_softmax(attn_logits, edge_index[0], dim=1) # [num_heads, nedge]
        weighted_v = attn_weights.unsqueeze(-1) * v  # [num_heads, nedge, head_dim]
        weighted_v = weighted_v.transpose(0, 1) # [nedge, num_heads, head_dim]
        assert weighted_v.shape == (edge_index.shape[1], self.num_heads, self.head_dim)
        weighted_v = weighted_v.reshape(-1, self.num_heads * self.head_dim)  # [nedge, hidden_dim]
        assert h.size(0)== node_features.shape[0]
        agg_features = scatter(weighted_v, edge_index[0], dim=0, reduce="sum",dim_size=node_features.shape[0])
        agg_features = self.output_linear(agg_features)
